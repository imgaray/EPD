...
Torque::Torque(double x, double y, double z) :
		Vec(x, y, z) {
}

Torque::Torque(const Vec& vector) :
		Vec(vector) {
}

Torque::Torque(const Torque& torque) :
		Vec((Vec&)torque) {
}
...
Vec& Torque::getAngularAcceleration(double inertia_moment) {
	Vec* accel = new Vec(x / inertia_moment, y / inertia_moment,
			z / inertia_moment);
	return *accel;
}
